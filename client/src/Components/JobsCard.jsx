...
function JobsCard(props) {
    ...
    async function handleClick(){
        if(userinfo.user && userinfo.user.status==="student"){
            Axios.post(
                "http://localhost:8080/addAuth",
                userinfo.user
            );
            navigate("/waitrequest");
        } else{
            navigate("/jobsview", {state: props.data});
        }
    }
    ...
    return (
        <div className="cardbox-new">
            <div className="card-title">
                ...
                <div className="card-applys"><a href={void (0)} onClick={handleClick}><AddTaskIcon /> Apply</a></div>
            </div>
            <div id="card-content">
                <table>
                    <thead>
